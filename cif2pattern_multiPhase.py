...
#V2 is an edit to make this a bit more of a robust tool
...
#do not edit the numbers below, These were empirically chosen to fit the model to 
#a reference measurement SNAP 48741: Nickel powder in a DAC at ambient pressure
#measured for 2.5 hours

noiseAmp = 1e-2
scale = 2.0e-4#1.5e2
outWS = 'Simulated Pattern'
offset = 0
...
#phase 2
cifPath = '/Users/66j/Downloads/IceVII_kuhs2.cif'
V = 37.6 #A^3
...
CreateWorkspace(OutputWorkspace=iceVII_sim,
DataX=x,
DataY=scale*(simSpec2)+bgdprof+noise+offset)    

#ticks
yloc = np.ones(len(d2))

CreateWorkspace(OutputWorkspace='Ticks: iceVII',
DataX=np.array(d2),
DataY=1.0*yloc)    

yloc = 1.25*np.ones(len(d3))

CreateWorkspace(OutputWorkspace='Ticks: SSSAHH-II',
DataX=np.array(d3),
DataY=yloc)    
